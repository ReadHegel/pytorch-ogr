...
class RunCfg:
    ...
    seed: int = 123
    device: str = "cpu"
    dtype: torch.dtype = torch.float64  # float64 znacznie stabilniejsze dla (quasi-)Newton
...
def minimize_with_ogr(fn: Callable[[Tensor], Tensor], x0: Tensor,
                      steps: int, tol_grad: float,
                      bounds: Tuple[Tuple[float, float], ...],
                      ogr_cfg: Dict[str, float]) -> Result:
...
def minimize_with_bfgs(fn: Callable[[Tensor], Tensor], x0: Tensor,
                       steps: int, tol_grad: float,
                       bounds: Tuple[Tuple[float, float], ...]) -> Result:
    x = x0.clone().detach().requires_grad_(True)
    opt = BFGS([x], lr=1.0)
    t0 = time.time()
    best_f = math.inf
...
def run_suite(cfg: RunCfg, names: Optional[List[str]] = None) -> None:
    ...
    print(f"Dimensions: {cfg.dim}, Restarts: {cfg.restarts}, Steps: {cfg.steps}, tol_grad: {cfg.tol_grad}\n")
    ...
    for name in sel:
        ...
        for _ in range(local_restarts):
            x0 = sample_uniform(bounds, device, dtype)
            try:
                res = minimize_with_ogr(wrap_fn, x0, local_steps, cfg.tol_grad, bounds, ogr_cfg)
        ...
        for _ in range(local_restarts):
            x0 = sample_uniform(bounds, device, dtype)
            res = minimize_with_bfgs(wrap_fn, x0, local_steps, cfg.tol_grad, bounds)
