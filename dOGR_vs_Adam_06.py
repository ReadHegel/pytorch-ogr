...
def build_dogr_optimizer(net: torch.nn.Module) -> optim.Optimizer:
    """Składa kompletny optimizer dOGR wraz z policy_net i jego optymalizatorem."""
    ...
    opt = dOGR(
        ...
        net=net,
        lr=dogr_config["lr"],
        beta=dogr_config["beta"],
        trust_factor=dogr_config["trust_factor"],
        nonlinear_clipping=dogr_config["nonlinear_clipping"],
        p_norm=dogr_config["p_norm"],
        p_eps=dogr_config["p_eps"],
        nn_policy=dogr_config["nn_policy"],
        policy_std=dogr_config["policy_std"],
        init_as_zeros=dogr_config["init_as_zeros"],
        differentiable=dogr_config["differentiable"],
    )
    return opt
...
def run_experiments_on_all_architectures():
    for net_name, net_cls in nets.items():
        ...
        for dataset_name, datamodule_class in datamodules.items():
            ...
            net_dogr = net_cls(input_dims=dm.dims, num_classes=dm.num_classes)
            optimizer_dogr = build_dogr_optimizer(net_dogr)
            run(
            ...
            for i, params in enumerate(param_combinations):
                ...
                torch.manual_seed(42)
                net_adam = net_cls(input_dims=dm.dims, num_classes=dm.num_classes)
...
def plot_final_results():
    ...
    for result_key, result_data in best_results.items():
        ...
        if dogr_log_path.exists():
            ...
            ax.plot(loss_dogr.index, loss_dogr.values, "-", label="dOGR (dostrojony)", linewidth=2.5)
